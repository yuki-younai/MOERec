...
class MOERec(BaseGraphModel):
    ...
    def get_embedding(self):
        ...
        user_muti=torch.randn(0).to(muti_int.device)  
        importance_sum=torch.randn(0).to(muti_int.device)  
        load_sum=torch.randn(0).to(muti_int.device) 
        for i in self.data_split:
            batch=self.data[i:i+32]
            muti=muti_int[batch]
            gate,load=self.noisy_top_k_gating(muti.view(muti.shape[0],-1),True)
            importance = gate.sum(0)
            dispatcher = SparseDispatcher(self.n_experts, gate)
            expert_inputs_user = dispatcher.dispatch(muti)
            ...
            user_muti=torch.cat([user_muti,expert_outputs],dim=0)
            importance_sum=torch.cat([importance_sum,importance],dim=0)
            load_sum=torch.cat([load_sum,load],dim=0)
            
        loss = self.cv_squared(importance_sum) + self.cv_squared(load_sum)
        print(loss)
        h_muti={'user':user_muti, 'item': h_item}
